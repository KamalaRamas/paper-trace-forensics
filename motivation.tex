...
In a failed interaction, such as the one seen in Figure~\ref{Failed_ex}, all the services which are not successful will be altering \ash{I did not understand this statement.}. The job of an SRE in this situation is to find the cause of the failure by:
\ash{Figure~\ref{Failed_ex} shows the  call-graph of a user interaction involving checkout of items for a cart. The edges marked in red indicate a rpc call that was unsuccessful.} 
\begin{itemize}
\item Figuring out which of the alerts \ash{we need to say failures and not alerts} are smokescreens to be ignored. \newline
...
\ash{I think we need to start the section with this explicit motivation argument and then support it with some evidence.}